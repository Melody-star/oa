<template>
	<view class="flex-col page">
		<view class="flex-col group">
			...
			<view class="flex-col list group_2 space-y-25" v-if="copyExamineList.length != 0">
				<view class="flex-col list-item" :key="i" v-for="(item, i) in temp" @click="gotoDetail(item)">
					<view class="flex-row justify-between group_3">
						<text class="font_2">{{item.processName}}</text>
						<image class="image_8"
					...
					<view class="flex-row justify-between group_6">
						<text class="font_3">申请人：{{item.originatorName}}</text>
						<text class="font_4" v-if="item.finishTime == null">审批中</text>
						<text class="font_4" v-else>已完成</text>
					</view>
				</view>
			</view>
			...
			gotoDetail(item) {
				uni.navigateTo({
					url: '/page_examine/mycreate-details/mycreate-details?taskId=' + item.taskId + '&procInsId=' +
						item.instanceId + '&deployId' + item.deploymentId + '&title=' + item.processName +
						'&createTime=' + item.createTime + '&copy=1'
				})
			},
			search(res) {
				if (res === '') {
					this.temp = this.copyExamineList
				} else {
					this.searchValue = this.copyExamineList
					if (res) {
						this.searchValue = this.copyExamineList.filter(item => item.processName.includes(res))
					}
