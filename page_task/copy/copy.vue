<template>
	<view class="flex-col space-y-20 page">
		...
		<view class="flex-col list" v-if="copyList.length != 0">
			<view class="flex-row space-x-9 list-item" :key="i" v-for="(item, i) in temp" @click="gotoDetail(item)">
				...
				<view class="flex-col group_2">
					...
					<view class="flex-row justify-between group_6">
						...
						<text class="font_6" v-if="item.taskDegreeOfCompletion === 0">已完成</text>
						<text class="font_6" v-if="item.taskDegreeOfCompletion === 1">未完成</text>
					</view>
				</view>
			</view>
		</view>
		...
		methods: {
			...
			search(res) {
				if (res === '') {
					...
					this.searchValue = this.copyList
					if (res) {
						this.searchValue = this.copyList.filter(item => item.taskName.includes(res))
					}
