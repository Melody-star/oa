<template>
	...
	<view class="screen-box" v-show="isShowscreen">
		...
		<view class="screen-box-item" @click="choose(1)">
			<text class="screen-box-item-text">已完成</text>
		...
		<view class="screen-box-item" @click="choose(2)">
			<text class="screen-box-item-text">未完成</text>
			...
			choose(value) {
				switch (value) {
					...
					case 1:
						...
						this.$emit('give-result', this.allList.filter(item => item.taskDegreeOfCompletion == 1))
					...
					case 2:
						...
						this.$emit('give-result', this.allList.filter(item => item.taskDegreeOfCompletion == 0))
